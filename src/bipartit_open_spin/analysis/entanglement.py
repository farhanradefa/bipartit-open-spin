...
def negativity(rho: Qobj) -> float:
    ...
    negative_evals = eigenvalues[eigenvalues < 0.0]
...
def negativity_trajectory(states: list[Qobj]) -> np.ndarray:
    ...
    """
    return np.array([negativity(state) for state in states], dtype=float)
